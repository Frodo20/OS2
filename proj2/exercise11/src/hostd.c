...
while (inputqueue || userjobqueue || CheckQueues(dispatcherqueues) >=0 || currentprocess) { //check all the queues
...
//         dequeue process from input queue and and enqueue on either
    while (inputqueue && inputqueue->arrivaltime <= timer)
    {
        ...
        process->status = PCB_READY; //set the pcb ready


//      a. Real-time queue so check out parameters before enqueueing
        if (process->priority == RT_PRIORITY){// If the process's priority is as same as RT_PRIORITY
            
            if (process->mbytes > RT_MEMORY_SIZE)//No enough memory left
            {
                fprintf(stderr,
                        "\nERROR - Real time job memory max %d Mbytes, job delete\n",
                        RT_MEMORY_SIZE);
                free(process);
                continue;
            }

            if (process->req.printers || process->req.scanners || process->req.modems || process->req.cds)// No enough I/O resources
            {
                fprintf(stderr,
                        "\nERROR - Real time job need more resources, job delete\n");
                free(process);
                continue;
            }

            if(rtmemory){process->memoryblock = rtmemory;} //If RT memory exists, assign RT memory to the processor

            else{
                if (!memChk(&memory, process->mbytes))//If the memory is not allocateable
                {
                    fprintf(stderr,
                            "\nERROR - No enough memory for Real time job, job delete\n");
                    free(process);
                    continue;
                }
        
                process->memoryblock = memAlloc(&memory, process->mbytes); //Allocate the memory block to the process
            }
            
            dispatcherqueues[RT_PRIORITY] = enqPcb(dispatcherqueues[RT_PRIORITY],process); //Enqueue to the appropriate level of queue
        }

//      b. user job queue - check out parameters before enqueueing
        else if (process->priority > RT_PRIORITY && process->priority <=LOW_PRIORITY) {//If priority is greater than RT_PRIORITY but lower than LOW_PRIORITY
            
            if (!memChkMax(process->mbytes)) // No available memory
            {
                fprintf(stderr,
                        "\nERROR - Job needs too much memory (%d Mbytes), job delete\n",
                        process->mbytes);
                free(process);
                continue;
            }

            if (!rsrcChkMax(process->req)) //No enough resources    
            {
                fprintf(stderr,
                        "\nERROR - Job needs too much resources, job delete\n");
                free(process);
                continue;
            }
                
            userjobqueue = enqPcb(userjobqueue, process); //If so, enqueue the process to the user job queue

        }
//           c. unknown priority - do nothing
        else{
            fprintf(stderr,
                    "\nERROR - Unknown priority: %d\n", process->priority);
            free(process);
            continue;
        }
...
//         While (head-of-user-job-queue.mbytes && resources can be allocated
    ...
    while (userjobqueue && memChk(&memory, userjobqueue->mbytes) && rsrcChk(&resources, userjobqueue->req)){
        // check if the process's MB and resources are allocateable
...
//      Meaning, enqueue to the appropriate level of queue
...
//          B. Free memory and resources we have allocated to the process;
            if (currentprocess->priority != RT_PRIORITY || !rtmemory){
                memFree(currentprocess->memoryblock);//free the memory
                rsrcFree(&resources, currentprocess->req); //free the resource
            }
...
//          the appropriate feedback queue;;
            ...
            dispatcherqueues[currentprocess->priority] = enqPcb(dispatcherqueues[currentprocess->priority], currentprocess); //Enqueue it to appropriate feedback queue
            currentprocess = NULL;
            }
        }
...
//              else start it (fork & exec)
//          c. Set it as currently running process;
            currentprocess = startPcb(currentprocess); //Start the currentprocess

        }

//       v. sleep for quantum;
        quantum = currentprocess && currentprocess->remainingcputime < QUANTUM ? currentprocess->remainingcputime : !(currentprocess) ? 1:QUANTUM;
        sleep(quantum);
