...
MabPtr memAlloc(MabPtr arena, int size)
{
    ...
    if (MabAlgorithm == NEXT_FIT) {
        ...
        // next fit
        if((m = memChk(next_mab,size)))
        {
            m->allocated = TRUE;
            next_mab = m->next;
        }
        // Not Found, and then search it from the beginning
        else{
            if((m = memChk(arena, size)))
            {
                if ((m=memSplit(m,size)))
                {
                    m->allocated = TRUE;
                    next_mab = m->next;
                }
    ...
    // Best Fit
        ...
        //keep finding until get the best
        MabPtr n = m->next;
        while (n)
        {
            if (!n->allocated && n->size >= size && n->size < m->size)
            {
                m=n; //replace
            }
            n=n->next;
    ...
    // Worst Fit
        ...
        //keep finding until get the worst
        MabPtr n =m->next;
        while(n)
        {
            if(!n->allocated && n->size >=size && n->size > m->size)
